...
async fn main() -> Result<(), reqwest::Error> {
	...
	let group_names = list_products_resp.groups.iter().map(|group| group.name.clone()).collect::<Vec<String>>();
	for group in list_products_resp.groups {
		let product_names = group.products.iter().map(|product| product.productshort.clone()).collect::<Vec<String>>();
			for product in group.products {
				let productshort = &product.productshort;
						...
						.map(|api_basic_info| {
							...
							async move {
								...
								match api_detail_resp {
									Ok(api_detail) => {
										let path = format!("{}/{}/{}/{}.json", gen_dir.display(), group_name, productshort, api_basic_info.name);
					...
					write_file(format!("{}/{}/{}/mod.rs", gen_dir.display(), group.name, productshort), mod_gen(&api_names).as_bytes()).await;
					break;
			}
			write_file(format!("{}/{}/mod.rs", gen_dir.display(), group.name), mod_gen(&product_names).as_bytes()).await;
	}

	write_file(format!("{}/mod.rs", &gen_dir.display()), feature_mod_gen(&group_names).as_bytes()).await;
...
async fn typify_gen_definition<P: AsRef<Path>>(path: P, name: &str, reference: &str, content: &[u8]) {
	...
	let companie_rs = path
		...
		.expect("parent")
		.join(format!("{}.rs", name));
	write_file(companie_rs, companie_rs_gen(name, reference).as_bytes()).await;
}

fn companie_rs_gen(name: &str, reference: &str) -> String {
	format!(r#"
